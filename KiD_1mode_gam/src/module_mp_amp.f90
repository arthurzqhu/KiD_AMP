...
if ( n_cat==1 ) then
  ...
  if (guess_try1%l1prm(1).ne.0 .or. guess_try1%l2prm(1).ne.0) then
...
if (n_cat == 1) then
  ...
  if (l_truncated) then
    ...
    m3 = dn(1)**3*(gamma(nu(1)+3)/gamma(nu(1)))
    mw = dn(1)**imomw*(gamma(nu(1)+imomw)/gamma(nu(1)))
    mx = dn(1)**imomx*(gamma(nu(1)+imomx)/gamma(nu(1)))
    ! print*, 'ml1', ml1
    ! print*, 'ml2', ml2
    ! print*, 'n01', n01
    ! print*, 'n02', n02
    ! print*, 'm3', m3 !dn(1)**3*(gamma(nu(1)+3)/gamma(nu(1)))
    ! print*, 'mw,mx', mw,mx
    print*, 'M0', M3p(1)/m3
    print*, 'Mw', mw*M3p(1)/m3
    print*, 'Mx', mx*M3p(1)/m3
    print*, 'm1frac', guess_best%l1_mfrac
    print*, 'dn1 dn2',guess_best%l1prm(1),guess_best%l2prm(1)
    print*, 'guess_best%error',guess_best%error
    print*, 'relaxw', relaxw
    ! call stopcm1
...
mcsm31 = mcs_1/m31
mcsm32 = mcs_2/m32
! print*, 'mcsm31,mcsm32', mcsm31,mcsm32
! print*, 'm1frac', (Mconsv_M3-mcsm32)/(mcsm31-mcsm32)
if (dn1 <= 0. .or. m31>infinity .or. mcs_1>infinity) then 
   m1frac = 0.
   dn1 = dnc_def
   ! m3 = m32
