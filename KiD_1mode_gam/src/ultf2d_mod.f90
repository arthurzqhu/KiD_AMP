...
module ultf2d_mod
  ...
  Use runtime
  Use switches
  Use column_variables, only: dx,field_mask
 ...
 contains
  ...
  subroutine ultf2d_interface(field, v, w, z, zw, rho, rhow, &
    ...
    do k=2,KKP-1
       dwrhodz(k)=(w(k+1,1)*rhow(k+1)-w(k,1)*rhow(k))/adz(k) !vertical wind div * rhow
    end do
    dwrhodz(KKP)=0.
    ...
    if (nx == 1) then
       ...
       do j=1,jjp
          ...
          field_adv(kkp,j) = (ardz(k)/rho(k))*(w(k-1,j)*FFLXB(j,K)*rhow(k-1)) &
               + ( v_ultf2d(j-1,k)*fflxl(j,k) - &
               v_ultf2d(j,k)*fflxl(j+1,k))/dx(j)
