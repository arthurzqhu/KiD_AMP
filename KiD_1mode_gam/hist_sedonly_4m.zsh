...
# config of the run
conf_basename='hist_sedonly_4m' # case/folder name. determined automatically if set empty
  ...
  do
     ...
     do
        ...
        if [[ ${bintype[$ibt]} = 'SBM' ]]; then
           imc1=2 # II moment for cloud
           imc2=5 # III moment for cloud
           imr1=2 # II moment for rain
        ...
        outdir=output/$(date +'%Y-%m-%d')/$config_fname/${ampORbin[$iab]}_${bintype[$ibt]}/
        ...
        do
           ...
           echo "${caselist[ic]}"
           cat > namelists/jobnml/${config_fname}_${ampORbin[$iab]}_${bintype[$ibt]}.nml << END
&mphys
! hydrometeor names
h_names='cloud','rain'
...
/
END
     ./bin/KiD_1D.exe namelists/jobnml/${config_fname}_${ampORbin[$iab]}_${bintype[$ibt]}.nml
#            done
#          done
#        done
#      done
