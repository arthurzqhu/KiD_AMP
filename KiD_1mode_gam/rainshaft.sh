...
# config of the run
mconfig='rainshaft' # case/folder name. determined automatically if set empty
caselist=(102) #(101 102 103 105 106 107)
case_num=${#caselist[@]}
ampORbin=("AMP" "BIN")
bintype=("TAU" "SBM")
tests2run_num=$((${#ampORbin[@]}*${#bintype[@]}))
dmlist=(3.e-5 1.e-4 3.e-4 1.e-3)
Nlist=(1.e7 1.e6 1.e5 1.e4)
...
iw=2
ia=100
for rh in 0.3 0.5 0.75 1
  for ((iraini=1; iraini<=${#dmlist}; iraini=iraini+1))
  do
    rs_dm=${dmlist[$iraini]}
    rs_N=${Nlist[$iraini]}
    echo dm=$rs_dm
