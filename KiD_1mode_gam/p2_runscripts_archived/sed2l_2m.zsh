...
# config of the run
conf_basename='sed2l_2m' # case/folder name. determined automatically if set empty
  ...
  do
   ...
   if [[ ${bintype[$ibt]} = 'SBM' ]]; then
      ...
      if [[ ${ampORbin[$iab]} = 'AMP' ]]; then
        ...
        outdir=/group/aigelgrp2/arthurhu/KiD/$(date +'%Y-%m-%d')/$config_fname/${ampORbin[$iab]}_${bintype[$ibt]}/
