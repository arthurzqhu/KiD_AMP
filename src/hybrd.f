     ...
     *                 qtf,wa1,wa2,wa3,wa4)
      ...
      double precision xtol,epsfcn,factor,fnorm2
...
c         (n*(n+1))/2.
c
c       qtf is an nutput array of length n which contains
...
c
      iflag = 1
      !print*,'x=',x,'fvec=',fvec,'info=',info,'nfev=',nfev
      !print*,'fjac=',fjac,'r=',r,'qtf=',qtf
...
c
         iflag = 2
         !print*,'fvec=',fvec!'x=',x!,'fvec=',fvec,'info=',info,'nfev=',nfev
         !print*,'fjac=',fjac,'r=',r,'qtf=',qtf
...
c
c            if (ratio .lt. p0001) go to 260
c Adele - don't declare success if fvec(1) equals 1. this is the
c value assigned to fvec when the parameters are out of bounds.
c fvec is wa4.
            if (ratio .lt. p0001 .or. wa4(1).eq.1.0) go to 260
c
...
c
            if (fnorm .le. xtol) info = 1
c            if (delta .le. xtol*xnorm .or. fnorm .eq. zero) info = 1
...
c 
      ...
      iflag = 0 
c Adele - do this final call to make sure that rxfinal has correct value
      if (nprint .gt. -1) call fcn(n,x,fvec,iflag)
      !print*,x,fvec,info,nfev,fjac,r,qtf
      return
c
