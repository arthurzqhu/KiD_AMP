...
imc2=8 # III moment for cloud
imr1=0 # II moment for rain
imr2=6 # III moment for rain
ia=300

for ((idt=0; idt<dt_num; idt++))
do
	echo "${dtlist[idt]}"
...
&control
mphys_scheme='amp'
dt=${dtlist[idt]}       !Timestep length (s)
dgstart=0.0      	!When to start diagnostic output
dg_dt=${dtlist[idt]}    !Timestep for diagnostic output
