...
# config of the run
mconfig_temp='collsed_spcr' # case/folder name. determined automatically if set empty
    ...
    do
      ...
      do
        ...
        if [[ ${ampORbin[$iab]} = 'AMP' ]]; then
          ...
          outdir=output/$(date +'%Y-%m-%d')/$mconfig/${ampORbin[$iab]}_${bintype[$ibt]}/dm${idm}/sp${isp_c}/
    	  ...
    	  do
    	    ...
    	    echo "${caselist[ic]}"
    	    cat > namelists/jobnml/${ampORbin[$iab]}_${bintype[$ibt]}_${idm}_${isp_c}.nml << END
&mphys
! hydrometeor names
h_names='cloud','rain'
...
/
END
     ./bin/KiD_1D.exe namelists/jobnml/${ampORbin[$iab]}_${bintype[$ibt]}_${idm}_${isp_c}.nml
#            done
#          done
#        done
#      done
