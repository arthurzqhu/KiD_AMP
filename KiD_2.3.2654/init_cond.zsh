...
# config of the run
mconfig_temp='dt_test' # case/folder name. determined automatically if set empty
    ...
    do
      ...
      do
        ...
        if [[ ${ampORbin[$iab]} = 'AMP' ]]; then
          ...
          outdir=output/$(date +'%Y-%m-%d')/$mconfig/${ampORbin[$iab]}_${bintype[$ibt]}/a${ia}w${iw}/dt${dt}/
    	  ...
    	  do
    	    ...
    	    echo "${caselist[ic]}"
    	    cat > namelists/jobnml/${ampORbin[$iab]}_${bintype[$ibt]}_${iw}_${ia}.nml << END
&mphys
! hydrometeor names
h_names='cloud','rain'
...
&control
mphys_scheme='amp'
dt=${dt}            !Timestep length (s)
dgstart=0.0       !When to start diagnostic output
dg_dt=${dt_diag}         !Timestep for diagnostic output
...
/
END
     ./bin/KiD_1D.exe namelists/jobnml/${ampORbin[$iab]}_${bintype[$ibt]}_${iw}_${ia}.nml
#            done
#          done
#        done
      done
